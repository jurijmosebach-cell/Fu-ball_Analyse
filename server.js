// server.js – optimierte, finale Version mit präzisen Berechnungen und Trend
import express from "express";
import path from "path";
import { fileURLToPath } from "url";

// Node 18+ hat fetch bereits
...
if (!FOOTBALL_DATA_KEY) {
  console.warn("⚠️ WARNING: FOOTBALL_DATA_API_KEY nicht gesetzt — API-Aufrufe liefern leere Ergebnisse.");
}
...
function computeBTTS(homeLambda, awayLambda, homeStats=null, awayStats=null) {
  ...
  if (homeStats && awayStats) {
    const formFactor = ((homeStats.avgGoalsFor + awayStats.avgGoalsFor)/2 - 
                        (homeStats.avgGoalsAgainst + awayStats.avgGoalsAgainst)/2) * 0.05;
...
async function fetchTeamStats(teamId, opts={limit:5}) {
  ...
  try {
    ...
    const stats = computeStatsFromMatches(data.matches?.slice(0,limit)||[]);
    teamStatsCache[teamId] = { timestamp: now, stats };
    await sleep(120);
    return stats;
  } catch(err) {
    console.warn("fetchTeamStats error", teamId, err.message||err);
...
async function fetchGamesFromAPI(){
  ...
  const matchEntries = (await Promise.all(leaguePromises)).flat();
