// server.js – voll optimierte Version mit H2H, Teamform & Trend
...
const teamStatsTTL = 60 * 60 * 1000;
const h2hCache = {}; // { 'homeId-awayId': {timestamp, stats} }
...
function poisson(k,lambda){ if(lambda<=0) return k===0?1:0; return Math.pow(lambda,k)*Math.exp(-lambda)/factorial(k); }

/* ---------- Verbesserte Match-Wahrscheinlichkeiten ---------- */
...
function computeBTTS(homeLambda,awayLambda,homeStats=null,awayStats=null){
  const pHome=1-poisson(0,homeLambda);
  const pAway=1-poisson(0,awayLambda);
  let btts=pHome*pAway;
...
async function fetchTeamStats(teamId,opts={limit:5}){
  ...
  const now=Date.now();
  if(cached && now-cached.timestamp<teamStatsTTL && cached.stats) return cached.stats;
...
async function fetchH2H(homeId,awayId,limit=5){
  ...
  const now=Date.now();
  if(cached && now-cached.timestamp<H2H_TTL) return cached.stats;

  try{
    const url=`https://api.football-data.org/v4/teams/${homeId}/matches?status=FINISHED`;
    const res=await fetch(url,{headers:{"X-Auth-Token":FOOTBALL_DATA_KEY}});
    if(!res.ok) throw new Error("H2H fetch failed");
    const data=await res.json();
    const h2hMatches=(data.matches||[]).filter(m=>m.awayTeam?.id===awayId || m.homeTeam?.id===awayId).slice(-limit);
    const stats=computeStatsFromMatches(h2hMatches);
    h2hCache[key]={timestamp:now,stats};
    await sleep(120);
    return stats;
  }catch(err){
    console.warn("H2H fetch error",homeId,awayId,err.message||err);
    h2hCache[key]={timestamp:now,stats:null};
    return null;
  }
}
...
function estimateXG(teamName,isHome,league="",teamStats=null){
  ...
  const LEAGUE_XG_FACTOR={
    ...
    "Campeonato Brasileiro Série A":0.92,"Primeira Liga":0.94,"Championship":0.98,"European Championship":0.90
  };
  const leagueFactor=LEAGUE_XG_FACTOR[league]||1.0;

  const strong=["Man City","Liverpool","Arsenal","Bayern","Real Madrid","PSG","Inter","Juventus","Barcelona"];
  const medium=["Newcastle","Aston Villa","Leipzig","Napoli","Atletico","Marseille","Leverkusen","Fiorentina"];
  ...
  if(teamStats && teamStats.played>0){
    ...
    if(strong.some(t=>teamName.includes(t))) adj+=0.35;
    else if(medium.some(t=>teamName.includes(t))) adj+=0.1;
    else if(weak.some(t=>teamName.includes(t))) adj-=0.25;
  }
...
async function fetchGamesFromAPI(){
  ...
  for(const entry of matchEntries){
    ...
    try{
      ...
      const homeName=m.homeTeam?.name||"Home", awayName=m.awayTeam?.name||"Away";
      const homeXG=estimateXG(homeName,true,leagueName,homeStats);
      const awayXG=estimateXG(awayName,false,leagueName,awayStats);

      // H2H-Faktor
      const h2hStats=homeId&&awayId?await fetchH2H(homeId,awayId,5):null;
      let h2hFactorHome=0,h2hFactorAway=0;
      if(h2hStats){
        h2hFactorHome=(h2hStats.avgGoalsFor-h2hStats.avgGoalsAgainst)/5;
        h2hFactorAway=(h2hStats.avgGoalsAgainst-h2hStats.avgGoalsFor)/5;
      }
      const adjustedHomeXG=homeXG*(1+0.1*h2hFactorHome);
      const adjustedAwayXG=awayXG*(1+0.1*h2hFactorAway);

      const outcome=computeMatchOutcomeProbs(adjustedHomeXG,adjustedAwayXG);
      const over25Prob=computeOver25Prob(adjustedHomeXG,adjustedAwayXG);
      const btts=computeBTTS(adjustedHomeXG,adjustedAwayXG,homeStats,awayStats);
